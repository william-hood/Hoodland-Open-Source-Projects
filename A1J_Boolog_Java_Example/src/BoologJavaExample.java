import hoodland.opensource.boolog.java.*;
...
public class BoologJavaExample {
    public static void main(String[] args) throws FileNotFoundException {
        ...
        File outputFile = new File(homeFolder + File.separator + "Documents" + File.separator + "Test Results" + File.separator + "Boolog Java Example.html");
        Boolog log = new Boolog("Boolog Example (from Java)", new PrintWriter(System.out), new PrintWriter(outputFile));
        ...
        try {
            ...
            log.showBoolog(subLog(check, inner));
    ...
    public static void testException(Boolog log) {
        try {
            makeAndPrint();
        } catch (Throwable thisProblem) {
    ...
    public static Boolog subLog(TestStruct check, TestStruct inner) {

        Boolog log = new Boolog("Click this to see one of Boolog's biggest tricks!", new PrintWriter(System.out), null);
        ...
        Boolog subLog = new Boolog("Click Here for some known issues with the Java wrapper for Boolog", new PrintWriter(System.out), null);
        subLog.info("Classes may need to be of public scope to shown by show() or showObject()", "▪");
        subLog.info("Because of the way the Java wrapper works, Boologs must be embedded with showBoolog() and not with show()", "▪");
        subLog.info("Because Boolog itself is written in Kotlin, primitive types displayed by showObject(), such as int or String, will show as \"kotlin.Int\" or \"kotlin.String\"", "▪");

        log.showBoolog(subLog, Constants.EMOJI_BOOLOG, "inconclusive_test_result");
        return log;
    }
}
